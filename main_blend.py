# python -u main_blend.py 2>&1 | tee CG1234_stdMultiplier_etaY_allDome_train6Re_test2Re_seed200.log
...
RANDOM_SEED = 200
RUN_TAG = "CG1234_stdMultiplier_etaY_allDome_train6Re_test2Re_seed200"
...
def write_metadata_file(case_data, train_case_data, test_case_data):
    metadata = {
        ...
        "closure_mapping": {
            ...
            "factor": (
                "factor = 1 for eta_y <= ACTIVATION_ETA_Y; "
                "factor = 1 + CORRECTION_AMPLITUDE*tanh((eta_y - ACTIVATION_ETA_Y)*raw) "
...
pset.renameArguments(ARG0="eta_y")

# Named constants avoid DEAP treating literal numbers as float-typed terminals.
pset.addTerminal(0.0, name="C_0")
pset.addTerminal(-0.5, name="C_m05")
pset.addTerminal(-1.0, name="C_m1")
pset.addTerminal(-2.0, name="C_m2")
...
def build_lm_from_yw_raw(raw, yw, eta_y):
    """
    ...
    For eta_y > ACTIVATION_ETA_Y:
        factor = 1 + CORRECTION_AMPLITUDE
                     * tanh((eta_y - ACTIVATION_ETA_Y)*raw)
        lm = lm_std * factor

    This preserves the standard model exactly in the tube/channel region
    and gives a smooth bounded GP correction in the outer region.
    """
    ...
    mask = eta_y > ACTIVATION_ETA_Y

    arg = (eta_y[mask] - ACTIVATION_ETA_Y) * raw[mask]
    factor[mask] = 1.0 + CORRECTION_AMPLITUDE * np.tanh(arg)

    lm = lm_std * factor

    print("raw min/max =", float(np.min(raw)), float(np.max(raw)), flush=True)
    print("factor min/max =", float(np.min(factor)), float(np.max(factor)), flush=True)
    print("lm_std min/max =", float(np.min(lm_std)), float(np.max(lm_std)), flush=True)
...
def evaluate_individual(individual, r_cg, z_cg, yw, eta_h, eta_y, ur_les, uz_les, case_path):
    ...
    try:
        ...
        lm, factor = build_lm_from_yw_raw(raw, yw, eta_y)
        print("factor min/max =", np.min(factor), np.max(factor), flush=True)
...
def initialize_template_from_population(population, case):
    ...
    for ind in population:
        ...
        lm, _ = build_lm_from_yw_raw(raw, yw, eta_y)
        lm_list.append(lm)
