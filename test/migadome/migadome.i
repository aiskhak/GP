...
# Best expression from search:
#   raw = tanh(-0.08485482588769211)
#
# In this Migadome implementation:
#   lm_std is computed by WallDistanceMixingLengthAux using delta_ml = 0.5
#   eta_y = yw / D0_eta, with D0_eta = 1.0
#
#   factor = 1
#            for eta_y < activation_eta_y_gp
#
#          = 1 + correction_amplitude_gp*tanh((eta_y-activation_eta_y_gp)*raw)
#            for eta_y >= activation_eta_y_gp
#
#   lm_gp = lm_std * factor
kappa_ml = 0.41

# Baseline mixing-length model scale:
delta_ml = 0.5
...
[AuxVariables]
  # This is the GP-corrected mixing length used by the Reynolds-stress kernels.
...
# Diagnostic: uncorrected baseline lm_std computed by WallDistanceMixingLengthAux
# using kappa_ml = 0.41 and delta_ml = 0.5.
  ...
  # Diagnostic: GP multiplier applied to lm_std.
  [gp_factor_aux_var]
    order  = CONSTANT
    family = MONOMIAL
    fv     = true
...
[AuxKernels]
  # Wall distance for eta_y diagnostics and GP activation
  [yw_aux_ker]
  ...
  # this preserves the original Migadome model with delta_ml = 0.5
  ...
  # GP multiplier:
  # eta_y = yw / D0_eta, where D0_eta = 1.0 is the nondimensional inlet diameter
  [gp_factor_aux_ker]
    type = ParsedAux
  ...
  # Actual GP-corrected mixing length:
  # standard Migadome lm multiplied by discovered GP factor
  [mixing_len_aux_ker]
    type = ParsedAux
...
[VectorPostprocessors]
  [vpp]
    ...
    variable = 'u v w pressure mixing_length_aux_var mixing_length_std_aux_var gp_factor_aux_var eddy_viscosity_aux_var yw_aux_var'
    sort_by = x
    execute_on = 'FINAL'
  []
